...
class MaskedAffineFlow(Flow):
    ...
    def __init__(self, b, t=None, s=None, in_res=64, c=3, latent_dim=256):
        """
        Constructor
        ...
        if t is None:
            ...
            self.add_module('t', t)
        self.cond = CondNetNF(latent_dim = self.latent_dim, in_res = self.in_res , c = self.c)
...
#     def forward(self, z, y):
#         z_masked = self.b * z
#         z_mask_cond = torch.concat(z_masked, y)
#         scale = self.s(z_mask_cond)
#         nan = torch.tensor(np.nan, dtype=z.dtype, device=z.device)
#         scale = torch.where(torch.isfinite(scale), scale, nan)
#         trans = self.t(z_mask_cond)
#         trans = torch.where(torch.isfinite(trans), trans, nan)
#         z_ = z_masked + (1 - self.b) * (z * torch.exp(scale) + trans)
#         log_det = torch.sum((1 - self.b) * scale, dim=list(range(1, self.b.dim())))
#         return z_, log_det
